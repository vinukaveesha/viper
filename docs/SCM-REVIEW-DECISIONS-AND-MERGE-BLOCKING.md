...
**Optional reply-dismissal.** If turned on in configuration (**§2**), an extra step can load the thread, run a small LLM, and—if it judges the reply sufficient—**omit that thread from counts for that run only** (nothing is written back to the SCM as “resolved”). If it judges the reply insufficient, the runner may post a brief follow-up on the thread where the API allows. Implemented for **GitHub**, **GitLab**, **Bitbucket Cloud**, and **Bitbucket Server / DC** (inline comment threads; gate exclusion uses the same `comment:{root_id}` stable id as the quality gate). **Gitea** does not implement this path yet (`skipped_no_capability`); recalculation still uses full SCM-derived counts.

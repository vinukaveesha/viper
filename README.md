...
AI-driven code review agent for CI/CD pipelines. Reviews pull request diffs, posts inline comments, and tracks resolved issues. Supports Gitea, GitHub, GitLab, Bitbucket and configurable LLMs (Gemini, OpenAI, Ollama).
...
- **Without Docker:** [Install locally](docs/QUICKSTART.md#option-2-run-without-docker-compose) and run `code-review review --owner <owner> --repo <repo> --pr <n> --head-sha <sha>` (set `SCM_*` and `LLM_*` via `.env` or environment).
...
The Compose-based Jenkins image preinstalls required plugins and can auto-seed credentials for local testing (see Quick Start).
